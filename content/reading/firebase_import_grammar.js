 ...
 * Imports real grammar questions per topic (23 topics) to Firestore
 * 
 * Usage:
...
// Real questions per topic - each with proper German grammar content
const QUESTIONS_DB = {
    b2_01: [
    ...
    b2_11: [
        ...
        { type: 'fill_blank', text: 'Er ___ das nicht machen sollen.', options: ['hätte', 'hätte', 'habe', 'werde haben'], answer: 'hätte', difficulty: 'hard' },
    ],
    b2_12: [
...
async function importGrammarQuestions() {
    ...
    for (const topic of B2_TOPICS) {
        ...
        const questions = baseQuestions.map((q, idx) => ({
            ...
            // Ensure options array exists for fill_blank type
            options: q.options || (q.type === 'true_false' ? ['Richtig', 'Falsch'] : [])
        }));
        ...
        // Import to grammarQuizBank collection
    ...
    console.log(`📊 ${B2_TOPICS.length} topics with varying questions each`);
}
...
async function main() {
    ...
    try {
        ...
        console.log('💡 Each topic has proper MCQ, true/false, and fill-blank questions');
